...
class ParseResult:
    ...
    answer_col: Optional[int] = None     # 1-based; where answers get written
...
def _pick_columns(rows: list[list[str]]) -> tuple[Optional[int], Optional[int]]:
    """Return (question_col_idx, answer_col_idx) as 0-based indices."""
    if not rows:
        return None, None
    ...
    # Answer column: a header named answer/response, else the column right of Q.
    a_col = None
    for c in range(min(n_cols, len(header))):
        if (header[c] or "").strip().lower() in {"answer", "response", "vendor response", "comments"}:
            a_col = c
            break
    if a_col is None:
        a_col = q_col + 1
...
def parse_xlsx(data: bytes) -> ParseResult:
    ...
    q_col, a_col = _pick_columns(rows)
